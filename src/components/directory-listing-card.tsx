...
type DirectoryListingCardProps = {
  ...
  /** When set, clicking the card (not bookmark / inline visit) opens the tool detail modal */
  onClick?: () => void;
};
...
export function DirectoryListingCard({
  ...
  return (
    <article
      className={`relative rounded-xl border border-zinc-800/80 bg-zinc-900/35 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.02)_inset] transition-colors hover:border-zinc-700/60 hover:bg-zinc-900/80 ${onClick ? "cursor-pointer" : ""}`}
      onClick={() => onClick?.()}
    >
      <button
        type="button"
